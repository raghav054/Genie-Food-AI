...
exports.analyzeAllRestaurants = catchAsync(async (req, res) => {
  ...
  for (const restaurant of restaurants) {
    ...
    try {
      ...
      console.log(`${restaurant.name} analyzed successfully`);
    } catch (err) {
      console.log(
        `Failed to analyze ${restaurant.name}:`,
        err.message
      );
    }
  }
  ...
  res.status(200).json({
    success: true,
    message: "All restaurants analyzed successfully",
  });
});
